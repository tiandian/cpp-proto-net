...
void CPortfolio::CheckOpenCancelLimit()
{
	...
	{
		msg = boost::str(boost::format("±¾´Î²ßÂÔ¿ª²Ö%dÊÖÒÑ´ïµ½ÉÏÏÞ%d") % m_openTimes % m_maxOpenPerStart);
		StopStrategyDueTo(msg);
	}
	else if(m_cancelTimes >= m_maxCancel)
	{
		msg = boost::str(boost::format("³·µ¥%d´ÎÒÑ´ïµ½ÉÏÏÞ%d") % m_cancelTimes % m_maxCancel);
		StopStrategyDueTo(msg);
	}
	else if(m_totalOpenTimes >= m_totalOpenLimit)
	{
		msg = boost::str(boost::format("×Ü¿ª²Ö%dÊÖÒÑ´ïµ½ÉÏÏÞ%d") % m_totalOpenTimes % m_totalOpenLimit);
		StopStrategyDueTo(msg);
	}
}
...
void CPortfolio::CheckForStop(const string& quoteUpdateTime)
{
	...
	else
	{
		...
		{
			string msg = boost::str(boost::format("²ßÂÔÒÑ×Ô¶¯Í£Ö¹ÓÚ%s") % m_targetEnd);
			StopStrategyDueTo(msg);
